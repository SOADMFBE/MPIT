window.onload = function () {
    ...
    function showQuestion(questionNumber) {
        document.querySelector('.question-text').innerHTML = quiz[step]['q'];
        let answer = '';
        for (let key in quiz[step]['a']) {
            answer += `<li data-v='${key}' class="answer-variant">${quiz[step]['a'][key]}</li>`;
        }
        document.querySelector('.answer').innerHTML = answer;
        document.querySelector('.num').innerHTML = step+1 + '.';
    }
    ...
    function showResult() {
        ...
        document.querySelector('.none').classList.replace('none', 'active');
    }
